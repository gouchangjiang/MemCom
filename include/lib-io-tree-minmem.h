...
void explore(Cnode * node, double available_memory,list<Cnode*> * L_init, schedule_t * S_init,  double & cut_value, list<Cnode*> & min_sub_cut, schedule_t & sub_schedule, double & Mpeak, int quiet, int depth,uint64_t & count);//added by Changjiang

void MinMemArray(int N,int root, double * nwghts, double * ewghts, double * mswghts, int * chstart, int * children,double MaxOutDeg, double & Required_memory, int * Schedule, int quiet, int & count);
