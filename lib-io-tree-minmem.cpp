...
#else
        //cout<<"-----Enter Explore-----\n";
        //cout<<"     Curent Node: "<<(*current_node)->GetId()<<"\n";
        //cout<<"     Available Memory: "<<(*current_node)->Mavail<<"\n";
        //cout<<"     Memory j: "<<m_j<<"\n";
        //cout<<"     L j are: ";
        /*for (list<Cnode*>::iterator ite=Lj.begin(); ite!=Lj.end(); ++ite) {
            cout<<(*ite)->GetId()<<" ";
        }
        cout<<"     S j are: ";
        for (list<int>::iterator ite=Sj.begin(); ite!=Sj.end(); ++ite) {
            cout<<(*ite)<<" ";
        }*/
        //cout<<"\n-----End-----\n";
...
#ifdef DEBUG_USING_MINMEM
    double MinMemRecurAlgorithm_timed(int N, int *prnts, double *nwghts, double *ewghts, double *mswghts, int *schedule,double * usec,int quiet,iter_node_t * minmem_trace)
#else
      double MinMemRecurAlgorithm_timed(int N, int *prnts, double *nwghts, double *ewghts, double *mswghts, int *schedule,double * usec,int quiet)
#endif
      {
        Ctree * tree = new Ctree(N,prnts,nwghts,ewghts,mswghts);
...
void MinMemArray(int N, int root, double * nwghts, double * ewghts, double * mswghts, int * chstart, int * children,double MaxOutDeg, double & Required_memory, int * Schedule, int quiet, uint64_t & count,int * prnts){
  double Cut_value = 0;
...
double MinMemArrayRecurAlgorithm(int N, int *prnts, double *nwghts, double *ewghts, double *mswghts, int *schedule){
  double Mr;
  double Mp;
  uint64_t count = 0;
  ...
  MinMemArray(N,root,nwghts,ewghts,mswghts,chstart,children,Mp, Mr, schedule, true, count,prnts);
...
double MinMemArrayRecurAlgorithm_timed(int N, int *prnts, double *nwghts, double *ewghts,double *mswghts, int *schedule,double * usec,int quiet){
  double Mr;
  double Mp;
  uint64_t count = 0;
  int * chstart,*chend,*children;
  ...
  *usec = -u_wseconds();
  MinMemArray(N,root,nwghts,ewghts,mswghts,chstart,children,Mp, Mr, schedule, quiet, count,prnts);
  *usec += u_wseconds();
