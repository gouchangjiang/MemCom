    ...
    if(OpenFile.good()){
      ...
      if(begin=='%'){
        ...
        if(!nodes_cnt_read){
          ...
          /*
          tree->GetNode(line_index)->SetParentId(parent);
          tree->GetNode(line_index)->SetEW(ew);
          tree->GetNode(line_index)->SetNW(nw);
          tree->GetNode(line_index)->SetId(line_index);
          tree->GetNode(line_index)->SetLabel(line_index);*/
            
          tree->GetNode(nb_of_nodes-line_index)->SetParentId(parent);
          tree->GetNode(nb_of_nodes-line_index)->SetEW(ew);
          tree->GetNode(nb_of_nodes-line_index)->SetNW(nw);
          tree->GetNode(nb_of_nodes-line_index)->SetMSW(MSw);
          tree->GetNode(nb_of_nodes-line_index)->SetId(nb_of_nodes-line_index);
            
          if(parent < nb_of_nodes+1){//parent!=0
            //tree->GetNode(parent)->AddChild(tree->GetNode(line_index));
            //tree->GetNode(line_index)->SetParent(tree->GetNode(parent));
              tree->GetNode(parent)->AddChild(tree->GetNode(nb_of_nodes-line_index));
              tree->GetNode(nb_of_nodes-line_index)->SetParent(tree->GetNode(parent));
          }

          if(parent==nb_of_nodes+1){//partent==0
            //tree->SetRootId(line_index);
            //tree->GetNode(line_index)->SetParentId(0);
              tree->SetRootId(nb_of_nodes-line_index);
              tree->GetNode(nb_of_nodes-line_index)->SetParentId(0);
          }
          line_index++;
        }
      }
...
double unload_largest_first_fit(Ctree* tree, vector<unsigned int> & unloaded_nodes,list<node_ew> & loaded_nodes,const double data_to_unload,double *ewghts){
    ...
    while ((largest_node!=loaded_nodes.end())&&(unloaded_data<data_to_unload)) {
        ...
        tree->GetNode(largest_node->first)->SetCut();//label this node as cut;
        //cout<<"Largest First cut edge "<<largest_node->first<<endl;
...
double unload_furthest_nodes(Ctree* tree, vector<unsigned int> & unloaded_nodes, list<node_sche> & loaded_nodes,const double data_to_unload, double * ewghts, bool divisible){
      ...
      while((far_node!=loaded_nodes.end()) && (unloaded_data < data_to_unload) ){
        ...
        //cout<<"LSNF cut edge "<<far_node->first<<endl;
          tree->GetNode(far_node->first)->SetCut();//label this node as cut
        loaded_nodes.pop_front();
      }										
...
double unload_furthest_first_fit(Ctree* tree, vector<unsigned int> & unloaded_nodes, list<node_sche> & loaded_nodes,const double data_to_unload, double * ewghts, bool divisible){
      ...
      while((far_node!=loaded_nodes.end()) && (unloaded_data < data_to_unload) ){
        ...
        if(local_data_to_unload >= data_to_unload){
          //cout<<"-------first fit success-------"<<endl;
          //cout<<"cut edge "<<far_node->first<<endl;
          unloaded_data += local_data_to_unload;
          unloaded_nodes.push_back(far_node->first);
            tree->GetNode(far_node->first)->SetCut();//label this node as cut
...
double IOCounter(Ctree* tree, int N, double * nwghts, double * ewghts, int * chstart,int * children, int * schedule, double available_memory,bool divisible,int quiet,unsigned int & com_freq, io_method_t method){
    ...
    for(int rank = N-1;rank>=1;rank--){
        ...
        if (cur_task_id!=0) {//0 means this node is on other subtrees
            /*if the node was unloaded*/
            vector<unsigned int>::iterator unloaded = find(unloaded_nodes.begin() , unloaded_nodes.end(), cur_task_id);
            if (unloaded!=unloaded_nodes.end()) {//find node cur_task_id unloaded
                //cout<<"edge "<<cur_task_id<<" is cutted"<<endl;
                //cout<<"com_freq before "<<com_freq<<", after addition ";
                ++com_freq;
                //cout<<com_freq<<endl;
                ...
                unsigned int subtree_size=temp.size();
                unsigned int subtree_size_copy=subtree_size;
                int* schedule_sub = new int[subtree_size+1];
                schedule_sub[subtree_size]=cur_task_id;
                for (long i=rank-1; i>=0;i-- ) {
                    vector<unsigned int>::iterator iter=find(temp.begin(), temp.end(), schedule[i]);
                    if (iter!=temp.end()) {
                        --subtree_size;
                ...
                double IO_sub = IOCounter(tree,subtree_size_copy+1, nwghts, ewghts, chstart,children, schedule_sub, available_memory, divisible, quiet, com_freq, method);
                ...
                if(data_to_unload > 0 )
                {
                    ...
                    double unloaded_data = 0.0;
                        ...
                        case FIRST_FIT:
                            ...
                            break;
                        case LARGEST_FIT:
                                //cout<<"idle space is not enough for "<<cur_task_id<<endl;
...
Ctree* SubtreeRooted(Cnode* node){
    ...
    if (node->IsLeaf()) {
        ...
        while (!visit_next.empty()) {
            if (!visit_next.back()->GetCut()) {// this child has not been cut
